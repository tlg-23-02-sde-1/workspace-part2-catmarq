...
//                .sorted( (emp1, emp2) -> emp1.getSalary().compareTo(emp2.getSalary()) )
    ...
    public void threeHighestPaid_sortByName() {
        List<Employee> result = allEmployees.stream()
//                .sorted((emp1, emp2) -> emp2.getSalary().compareTo(emp1.getSalary()))   // descending order
                .sorted(Comparator.comparing(Employee::getSalary).reversed())             // simplified option
                .limit(3)
    ...
    public void allEmployeesHired2000_orLater() {
        boolean hiredAfter2000 = allEmployees.stream()
                .allMatch(emp -> emp.getHireDate().getYear() >= 2000);

        System.out.println(hiredAfter2000);
    }
